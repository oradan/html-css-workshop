...
document.addEventListener("DOMContentLoaded", function () {
  ...
  addToCartButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      console.log(
        "SIBLING COMMUNICATION: Add to cart button component updated counter component"
      );
    });
  });
  ...
  function toggleFavorite(carItem, heartIcon, button) {
    if (carItem.classList.contains("favorited")) {
      // Remove from favorites
      carItem.classList.remove("favorited");
      if (heartIcon) heartIcon.style.color = "";
      console.log("Car removed from favorites");
      console.log(
        "PARENT-CHILD COMMUNICATION: Parent component updated child button state"
      );
    } else {
      // Add to favorites
      carItem.classList.add("favorited");
      if (heartIcon) heartIcon.style.color = "#e74c3c"; // Red heart only
  ...
  favoriteButtons.forEach((button) => {
    button.addEventListener("click", function () {
      ...
      toggleFavorite(carItem, heartIcon, button);
    });
  });

  // Add event listeners to each favorite heart overlay
  favoriteHearts.forEach((heart) => {
    heart.addEventListener("click", function () {
      const carItem = this.closest(".car-item");
      const favoriteButton = carItem.querySelector(".btn.bg-gray i");
      toggleFavorite(carItem, favoriteButton, null);
    });
  });
  ...
  carItems.forEach((carItem) => {
    ...
    optionChecks.forEach((checkbox) => {
      checkbox.addEventListener("change", function () {
        ...
        console.log(
          `Option "${optionName}" ${
            this.checked ? "added" : "removed"
          }: $${optionPrice.toLocaleString()}`
        );
        console.log(
          "SMART COMPONENT: Car component calculated new total price"
        );
      });
    });
  });
  ...
  console.log(
    "- In Angular: Smart components use services, dumb components use @Input/@Output"
  );
});
